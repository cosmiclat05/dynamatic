...
std::vector<std::string> BlifData::getNodesInOrder() const{
...
void BlifData::traverseUtil(const std::string& node, std::set<std::string>& visitedNodes) {
    if (std::find(m_nodesTopologicalOrder.begin(), m_nodesTopologicalOrder.end(), node) != m_nodesTopologicalOrder.end()) {
        return;
    }
    ...
    for (const auto& fanin : m_nodeFanins.at(node)) {
        if (std::find(m_nodesTopologicalOrder.begin(), m_nodesTopologicalOrder.end(), fanin) == m_nodesTopologicalOrder.end() && visitedNodes.count(fanin) > 0) {
            //throw std::runtime_error("Cyclic dependency detected!");
        }
        else if (std::find(m_nodesTopologicalOrder.begin(), m_nodesTopologicalOrder.end(), fanin) == m_nodesTopologicalOrder.end())
        {
...
void BlifData::traverseNodes() {
    ...
    for (const auto& node : getPrimaryOutputs()){
...
BlifData BlifParser::parseBlifFile(const std::string &filename) {
    ...
    while (std::getline(file, line)){
        ...
        if (line.find(".model") == 0) {
            ...
            for (int i = 0; i < nodes.size() - 1; i++) { //add fanins of the fanout node
